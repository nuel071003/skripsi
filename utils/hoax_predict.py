"""
=============================================================
  hoax_predict.py  –  Utils Deteksi Hoaks
  Tugas : Memuat model BART Hoaks dan memprediksi apakah
          sebuah isi berita mengandung hoaks atau tidak.
=============================================================

Cara kerja model:
  - Arsitektur : BartForSequenceClassification (2 kelas)
  - LABEL_0    : Non-Hoaks (berita valid)
  - LABEL_1    : Hoaks
  - Input      : Isi / konten berita (string)
  - Output     : label (str) dan confidence (float 0-100)

Catatan:
  Karena isi berita bisa sangat panjang, konten akan dipotong
  hingga MAX_LENGTH token agar sesuai batas input BART.
"""
...
def muat_model_hoax():
    """
    Memuat tokenizer dan model hoaks dari folder lokal.
    Fungsi ini hanya perlu dipanggil SATU KALI saat aplikasi mulai.

    Model disimpan di: models/model_hoax/
    """
    global _tokenizer, _model

    # Jika sudah dimuat sebelumnya, lewati
    ...
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            f"[ERROR] Folder model hoaks tidak ditemukan: {MODEL_PATH}\n"
            f"Pastikan folder 'models/model_hoax/' ada dan berisi file model."
        )

    print(f"[INFO] Memuat model hoaks dari: {MODEL_PATH}")
    ...
    # Set ke mode evaluasi (nonaktifkan dropout)
    _model.eval()
